...
function AddWorkType() {
  ...
  const [workTypevalidationError, setWorkTypeValidationError] =
    useState("Required Field");
  ...
  const handleWorkTypeChange = (newWorkType) => {
    ...
    const isEditDataAvailable =
      editData && editData[0] && editData[0][newWorkType.name];

    setValidationError((prevState) => ({
      ...prevState,
      [newWorkType.name]: isEditDataAvailable
        ? ""
        : newWorkType.value.trim() !== ""
        ? ""
        : "Required Field",
    }));
  };
